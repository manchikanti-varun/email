...
//                      (technical capability to receive mail — NOT about the person)
//   deliverabilityScore : 0-100 technical score (explainable, NOT penalised for
//                         being role-based / shared)
...
//   reasons          : plain-language explanation of the classification
...
// An evidence item is an observable technical check outcome shown to the user.
//   status: 'pass' | 'warn' | 'fail' | 'info'
function evidence(status, label) {
  return { status, label };
}
function risk(code, label, detail) {
  return { code, label, detail };
}

const DELIVERABILITY = {
  DELIVERABLE: 'deliverable',     // technically confirmed able to receive mail
  UNDELIVERABLE: 'undeliverable', // strong technical evidence it cannot
  RISKY: 'risky',                 // deliverable-ish but a technical caveat exists (catch-all)
  UNKNOWN: 'unknown',             // insufficient evidence either way
};
const CONFIDENCE = { HIGH: 'high', MEDIUM: 'medium', LOW: 'low', UNKNOWN: 'unknown' };
const ACTION = { KEEP: 'keep', REVIEW: 'review', REMOVE: 'remove', REVERIFY: 'reverify' };

export async function verifyEmail(rawEmail, options = {}) {
  const useProvider = options.useProvider !== false;
  const email = normalizeEmail(rawEmail);

  const ev = [];            // technical evidence
  const risks = [];         // risk characteristics (never fatal on their own)
  const reasons = [];       // explanation of the final classification

  // ===================================================================== //
  //  STEP 1 — SYNTAX (hard technical fact)                                //
  // ===================================================================== //
  ...
  // ===================================================================== //
  //  STEP 2 — RESERVED / DOCUMENTATION DOMAIN (hard technical fact)        //
  // ===================================================================== //
  ...
  // ===================================================================== //
  //  STEP 3 — DISPOSABLE (strong technical/risk fact)                     //
  // ===================================================================== //
  ...
  if (disposable) {
    ...
    // Disposable is a strong reason to remove regardless of anything else.
    return finalize(email, {
      deliverability: DELIVERABILITY.UNDELIVERABLE,
  ...
  // ===================================================================== //
  const role = isRole(local);
  if (role) {
    // Recorded purely as a risk characteristic. It does NOT reduce the
    // deliverability score and does NOT make the address unhealthy.
  ...
  // ===================================================================== //
  //  STEP 5 — DOMAIN / DNS / MX (technical capability)                    //
  // ===================================================================== //
  ...
  // ===================================================================== //
  //  STEP 6 — SMTP MAILBOX PROBE (direct evidence, where possible)        //
  // ===================================================================== //
  const smtp = await smtpCheck(email, dnsResult.mxHosts);
  let catchAll = false;
  let smtpProbed = false;        // did we actually complete an SMTP conversation?
  let smtpUnavailable = false;   // could we not probe at all (blocked/skipped)?
  ...
  if (smtp.skipped) {
    ...
    // Could connect to nothing — treat as inconclusive, NOT as proof of failure.
    smtpUnavailable = true;
  ...
  if (useProvider && dnsResult.domainExists && localInconclusive) {
    try {
      const p = await getProvider().verify(email, {
  ...
  // ===================================================================== //
  //  COMBINE — independent dimensions, then a single recommended action   //
  // ===================================================================== //
  const facts = {
...
function assessDeliverability(f) {
  ...
  if (f.catchAll) return DELIVERABILITY.RISKY;        // accepts mail, but per-mailbox unconfirmable
  // Domain + MX are healthy but the mailbox itself is unconfirmed.
  // This is UNKNOWN — not undeliverable. Lack of evidence is not negative evidence.
  return DELIVERABILITY.UNKNOWN;
}

// ---- Dimension 2: strength of evidence ------------------------------------
function assessConfidence(f) {
  if (f.mailboxConfirmed || f.mailboxRejected) return CONFIDENCE.HIGH; // direct mailbox evidence
  if (f.catchAll) return CONFIDENCE.MEDIUM;   // we know the domain behaviour, not the mailbox
  if (f.greylisted) return CONFIDENCE.LOW;
  if (f.smtpUnavailable) {
    // We have domain+MX but could not probe the mailbox at all.
    return f.hasMx ? CONFIDENCE.MEDIUM : CONFIDENCE.LOW;
  }
  return CONFIDENCE.LOW;
}

// ---- Technical deliverability score (0-100) -------------------------------
// Reflects TECHNICAL capability + confidence only. It is NOT reduced for being
// role-based or a shared mailbox — those are characteristics, not defects.
function scoreDeliverability(f, deliverability) {
  ...
  if (deliverability === DELIVERABILITY.RISKY) return 55;    // catch-all: real but unconfirmable

  // UNKNOWN: base on how much healthy technical evidence we do have.
  let score = 50;                 // syntax + domain established
  if (f.hasMx) score += 20;       // dedicated mail servers exist
  else if (f.aOnly) score += 5;
  if (f.greylisted) score -= 5;   // transient, minor
  return Math.max(0, Math.min(75, score)); // never claim high confidence without mailbox proof
}

// ---- Recommended action: combination of ALL dimensions --------------------
function recommendAction({ deliverability, confidence, facts }) {
  if (deliverability === DELIVERABILITY.UNDELIVERABLE) return ACTION.REMOVE;
  // Deliverable — KEEP, even if role-based/shared (that's just a note).
  if (deliverability === DELIVERABILITY.DELIVERABLE) return ACTION.KEEP;
  // Catch-all: technically accepts mail but mailbox unconfirmable → review/reverify.
  if (deliverability === DELIVERABILITY.RISKY) return ACTION.REVIEW;
  // UNKNOWN: if we simply couldn't probe, ask to reverify (don't punish).
...
function buildReasons({ reasons, deliverability, confidence, facts }) {
  ...
  // Characteristics are stated as facts, never as health judgments.
  if (facts.role) {
    reasons.push(
...
function recommendationText({ deliverability, confidence, recommendedAction, facts }) {
...
function finalize(email, r) {
  // A single, explainable overall score is retained for convenience, but it is
  // simply the deliverability score — it is NOT penalised for characteristics
  // like being role-based. Consumers should prefer the individual dimensions.
  return {
    email,
    // Independent dimensions (the source of truth):
    ...
    // ---- Backward-compatible fields (derived, do not add new meaning) ----
...
// Map the recommended action to the legacy classification bucket used by the
// storage/UI layers: keep→safe, review→review, reverify→unknown, remove→remove.
