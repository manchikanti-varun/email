...
export function explainEmail({ contact, mode = 'simple' }) {
  ...
  else if (deliverability === 'risky' || catchAll) simple = 'This email is on a domain that accepts mail for any address, so the specific mailbox cannot be independently confirmed. Consider reviewing it before a large campaign.';
